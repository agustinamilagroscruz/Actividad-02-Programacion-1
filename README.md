...
4. **Normalizar y transformar un texto:** Limpia espacios en los extremos con `strip()`, aplica un criterio uniforme en minúsculas con `lower()` y reemplaza una palabra o expresión mediante `replace()`, mostrando la comparación entre el original y el transformado (sin mutar la matriz original).
